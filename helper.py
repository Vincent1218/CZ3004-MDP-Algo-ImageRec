...
def command_generator(states, obstacles):
    ...
    for i in range(1, len(commands)):
        # If both commands are b0
        # print(commands[i])
        if commands[i].startswith("b") and compressed_commands[-1].startswith("b"):
            # Get the number of steps of previous command and add 10 to it
            steps = int(compressed_commands[-1][1:]) + 10
            if steps < 100:
                compressed_commands[-1] = "b0{}".format(steps)
            elif steps < 201:
                compressed_commands[-1] = "b{}".format(steps)
            else:
                raise Exception("Invalid distance, greater than arena")
            continue

        # If both commands are f
        elif commands[i].startswith("f") and compressed_commands[-1].startswith("f"):
            # Get the number of steps of previous command
            steps = int(compressed_commands[-1][1:]) + 10
            if steps < 100:
                compressed_commands[-1] = "f0{}".format(steps)
            elif steps < 201:
                compressed_commands[-1] = "f{}".format(steps)
            else:
                raise Exception("Invalid distance, greater than arena")
            continue
